...
class Trainer(BaseTrainer):
    ...
    def init_fn(self):
        ...
        self.summary_writer = wandb.init(project=self.options.wandb_project, name=self.options.name, entity='zzweng')
    ...
    def keypoint_loss(self, pred_keypoints_2d, gt_keypoints_2d, openpose_weight, gt_weight, reduce=True):
        ...
        if reduce:
            ...
            loss = (conf * self.criterion_keypoints(pred_keypoints_2d, gt_keypoints_2d[:, :, :-1])).mean((1,2))
        return loss

    def keypoint_3d_loss(self, pred_keypoints_3d, gt_keypoints_3d, has_pose_3d, reduce=True):
        """Compute 3D keypoint loss for the examples that 3D keypoint annotations are available.
        The loss is weighted by the confidence.
        """
        if gt_keypoints_3d.shape[1] == 25:
        ...
        if len(gt_keypoints_3d) > 0:
            if gt_keypoints_3d.shape[1] == 24:
                ...
                gt_pelvis = gt_keypoints_3d[:, 8,:]
                pred_pelvis = pred_keypoints_3d[:, 8,:]
            gt_keypoints_3d = gt_keypoints_3d - gt_pelvis[:, None, :]
            pred_keypoints_3d = pred_keypoints_3d - pred_pelvis[:, None, :]
            if reduce:
                return (conf * self.criterion_keypoints(pred_keypoints_3d, gt_keypoints_3d)).mean()
            else:
                return (conf * self.criterion_keypoints(pred_keypoints_3d, gt_keypoints_3d)).mean((1,2))
        else:
            if reduce:
                return torch.tensor(0).float().to(self.device)
#                 return torch.FloatTensor(1).fill_(0.).to(self.device)
            else:
                return torch.FloatTensor(gt_keypoints_3d.shape[0]).fill_(0.).to(self.device)

    def shape_loss(self, pred_vertices, gt_vertices, has_smpl, reduce=True):
        ...
        if len(gt_vertices_with_shape) > 0:
            if reduce:
                ...
                return self.criterion_shape_nr(pred_vertices_with_shape, gt_vertices_with_shape).mean((1, 2))
        else:
            if reduce:
                return torch.FloatTensor(1).fill_(0.).to(self.device)
            else:
                return torch.FloatTensor(gt_vertices.shape[0]).fill_(0.).to(self.device)
            
    def smpl_losses(self, pred_rotmat, pred_betas, gt_pose, gt_betas, has_smpl, reduce=True):
        ...
        if len(pred_rotmat_valid) > 0:
            if reduce:
                ...
                loss_regr_pose = self.criterion_regr_nr(pred_rotmat_valid, gt_rotmat_valid).mean((1,2,3))
                loss_regr_betas = self.criterion_regr_nr(pred_betas_valid, gt_betas_valid).mean(1)
        else:
            if reduce:
                loss_regr_pose = torch.FloatTensor(1).fill_(0.).to(self.device)
                loss_regr_betas = torch.FloatTensor(1).fill_(0.).to(self.device)
            else:
                loss_regr_pose = torch.FloatTensor(pred_betas.shape[0]).fill_(0.).to(self.device)
                loss_regr_betas = torch.FloatTensor(pred_betas.shape[0]).fill_(0.).to(self.device)
    ...
    def train_summaries(self, input_batch, output, losses):
        ...
        def overlay_kp(keypoints): # visualize the 2D keypoints
            op_colors = [
                [255,     0,    85],
                [255,     0,     0], 
                [255,    85,     0], 
            ...
            overlayed = []
            keypoints[:,:,:-1] = 0.5 * constants.IMG_RES * (keypoints[:, :, :-1] + 1)
            for i in range(images.shape[0]):
                pimg = ToPILImage()(images[i].cpu())
                draw = ImageDraw.Draw(pimg)
                for k in range(25):
                    leftUpPoint = (keypoints[i,k,0].item(), keypoints[i,k,1].item())
                    rightDownPoint = (keypoints[i,k,0].item()+10, keypoints[i,k,1].item()+10)
                    draw.ellipse([leftUpPoint, rightDownPoint], fill=tuple(op_colors[k]), width=2)
                overlayed.append(ToTensor()(pimg))
            return torch.stack(overlayed).to(images.device)
            
        images = overlay_kp(input_batch['keypoints'])
        
        if 'textured_images' in output.keys():
            textured_imgs = self.renderer.visualize_images(images, output['textured_images'])
            self.summary_writer.log({'real/pred_texture': wandb.Image(textured_imgs)})
        ...
        self.summary_writer.log({'real/pred_shape': wandb.Image(images_pred)})
        if self.options.run_smplify:
            self.summary_writer.log({'real/opt_shape': wandb.Image(images_opt)})
        for loss_name, val in losses.items():
            self.summary_writer.log({'real/'+loss_name: val})
