...
class BaseDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Hack (Jen, Aug10): for seedlings, point to our saved copy of the test images.
        # /scratch/groups/syyeung/seedlings/month6/03_06_fps3/images/frame005017.jpg
        # info_str, _, framename = imgname.split('/')[-3:]
        # imgname = join('seedlings_anns/seedlings_imgs/{}_{}'.format(info_str[:-5], framename))
        try:
            img = cv2.imread(imgname)[:,:,::-1].copy()
            if self.parse_random_bg:
                random_bg = RandomCrop(scale*200, pad_if_needed=True, padding_mode='edge')(ToPILImage()(img))
